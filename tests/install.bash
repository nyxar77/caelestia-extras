#!/usr/bin/env bash
set -euo pipefail

repo_dir=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
test_dir=$(mktemp -d "${TMPDIR:-/tmp}/caelestia-extras-install-test.XXXXXX")
trap 'rm -rf "$test_dir"' EXIT HUP INT TERM

fake_bin="$test_dir/fake-bin"
mkdir -p "$fake_bin"
mkdir -p "$test_dir/tmp"
cat > "$fake_bin/go" <<'EOF'
#!/bin/sh
set -eu

output=
while [ "$#" -gt 0 ]; do
  case "$1" in
    -o)
      output=$2
      shift
      ;;
  esac
  shift
done

[ -n "$output" ]
if [ "${TEST_INTERRUPT_GO:-}" = 1 ]; then
  kill -INT "$PPID"
fi
printf '%s\n' '#!/bin/sh' 'exit 0' > "$output"
chmod +x "$output"
EOF
chmod +x "$fake_bin/go"

cat > "$fake_bin/systemctl" <<'EOF'
#!/bin/sh
set -eu

printf '%s\n' "$*" >> "$TEST_SYSTEMCTL_LOG"
if [ "${TEST_INTERRUPT_SYSTEMCTL:-}" = 1 ]; then
  kill -INT "$PPID"
fi
EOF
chmod +x "$fake_bin/systemctl"

export HOME="$test_dir/home"
export XDG_CONFIG_HOME="$test_dir/config"
export XDG_DATA_HOME="$test_dir/data"
export XDG_STATE_HOME="$test_dir/state"
export XDG_BIN_HOME="$test_dir/bin"
export TMPDIR="$test_dir/tmp"
export TEST_SYSTEMCTL_LOG="$test_dir/systemctl.log"
export PATH="$fake_bin:$PATH"

run_install() {
  "$repo_dir/scripts/install.sh" "$@"
}

if HOME="$test_dir/invalid/home" \
  XDG_CONFIG_HOME="$test_dir/invalid/config" \
  XDG_DATA_HOME="$test_dir/invalid/data" \
  XDG_STATE_HOME="$test_dir/invalid/state" \
  XDG_BIN_HOME="$test_dir/invalid/bin" \
  "$repo_dir/scripts/install.sh" --enable cursor >/dev/null 2>&1; then
  printf '%s\n' "installer accepted an unsupported --enable value" >&2
  exit 1
fi
test ! -e "$test_dir/invalid/bin/caelestia-extras"

run_install install

config_file="$test_dir/config/caelestia-extras/config.toml"
test -x "$test_dir/bin/caelestia-extras"
test -f "$config_file"
test -L "$test_dir/config/caelestia/templates/gtk-portal.css"
! grep -q 'window > overlay > paned' "$test_dir/config/caelestia-extras/managed/templates/gtk-portal.css"
test -L "$test_dir/config/caelestia/templates/gtk.css"
test -L "$test_dir/config/caelestia/templates/gtk4.css"
test -L "$test_dir/config/caelestia/templates/gtk3-adwaita.css"
test -L "$test_dir/config/caelestia/templates/imv.conf"
grep -qx 'background={{ surface.hex }}' "$test_dir/config/caelestia-extras/managed/templates/imv.conf"
grep -qx 'overlay_text_color={{ onSurface.hex }}' "$test_dir/config/caelestia-extras/managed/templates/imv.conf"
grep -qx 'overlay_background_color={{ surfaceContainerLowest.hex }}' "$test_dir/config/caelestia-extras/managed/templates/imv.conf"
! grep -q '^:root' "$test_dir/config/caelestia-extras/managed/templates/gtk.css"
grep -q '^@define-color theme_bg_color @window_bg_color;$' "$test_dir/config/caelestia-extras/managed/templates/gtk.css"
grep -q '^:root' "$test_dir/config/caelestia-extras/managed/templates/gtk4.css"
test -L "$test_dir/config/gtk-3.0/gtk.css"
test -L "$test_dir/config/gtk-4.0/gtk.css"
test -L "$test_dir/config/imv/config"
test "$(readlink "$test_dir/config/imv/config")" = "$test_dir/state/caelestia/theme/imv.conf"
test "$(readlink "$test_dir/config/gtk-3.0/gtk.css")" = "$test_dir/state/caelestia/theme/gtk.css"
test "$(readlink "$test_dir/config/gtk-4.0/gtk.css")" = "$test_dir/state/caelestia/theme/gtk4.css"
test -L "$test_dir/data/themes/Caelestia-GTK3/gtk-3.0/gtk.css"
test "$(readlink "$test_dir/data/themes/Caelestia-GTK3/gtk-3.0/gtk.css")" = "$test_dir/state/caelestia/theme/gtk3-adwaita.css"
test -L "$test_dir/data/themes/Caelestia-GTK3/gtk-3.0/base-dark.css"
grep -q '^@import url("base-{{ mode }}.css");$' "$test_dir/config/caelestia-extras/managed/templates/gtk3-adwaita.css"
grep -qx '  "qssFilePath": "themeStyle.css",' "$test_dir/config/caelestia/templates/prismlauncher.json"
test -L "$test_dir/config/caelestia/templates/prismlauncher.qss"
test -L "$test_dir/config/caelestia/templates/breeze-caelestia.colors"
grep -q '^\[ColorEffects:Disabled\]$' "$test_dir/config/caelestia-extras/managed/templates/breeze-caelestia.colors"
grep -q '^\[Colors:Header\]\[Inactive\]$' "$test_dir/config/caelestia-extras/managed/templates/breeze-caelestia.colors"
grep -q '^\[WM\]$' "$test_dir/config/caelestia-extras/managed/templates/breeze-caelestia.colors"
test -L "$test_dir/config/systemd/user/caelestia-extras-watch.service"
test -L "$test_dir/data/themes/Caelestia-Portal/gtk-4.0/base-dark.css"
test -L "$test_dir/data/PrismLauncher/themes/caelestia-breeze/theme.json"
test -L "$test_dir/data/PrismLauncher/themes/caelestia-breeze/themeStyle.css"

if TEST_INTERRUPT_GO=1 \
  HOME="$test_dir/cancel/home" \
  XDG_CONFIG_HOME="$test_dir/cancel/config" \
  XDG_DATA_HOME="$test_dir/cancel/data" \
  XDG_STATE_HOME="$test_dir/cancel/state" \
  XDG_BIN_HOME="$test_dir/cancel/bin" \
  TMPDIR="$test_dir/tmp" \
  "$repo_dir/scripts/install.sh" >/dev/null 2>&1; then
  printf '%s\n' "installer did not stop on SIGINT" >&2
  exit 1
fi
test ! -e "$test_dir/cancel/bin/caelestia-extras"
test ! -e "$test_dir/cancel/config/caelestia-extras/config.toml"

if TEST_INTERRUPT_SYSTEMCTL=1 \
  HOME="$test_dir/post-apply/home" \
  XDG_CONFIG_HOME="$test_dir/post-apply/config" \
  XDG_DATA_HOME="$test_dir/post-apply/data" \
  XDG_STATE_HOME="$test_dir/post-apply/state" \
  XDG_BIN_HOME="$test_dir/post-apply/bin" \
  "$repo_dir/scripts/install.sh" --enable all >/dev/null 2>&1; then
  printf '%s\n' "installer ignored SIGINT after applying files" >&2
  exit 1
fi
test -x "$test_dir/post-apply/bin/caelestia-extras"

printf '%s\n' '# user-owned' > "$config_file"
ln -sfn "$test_dir/state/caelestia/theme/gtk.css" "$test_dir/config/gtk-4.0/gtk.css"
run_install update
grep -qx '# user-owned' "$config_file"
test "$(readlink "$test_dir/config/gtk-4.0/gtk.css")" = "$test_dir/state/caelestia/theme/gtk4.css"

portal_template="$test_dir/config/caelestia/templates/gtk-portal.css"
rm "$portal_template"
printf '%s\n' '/* user-owned */' > "$portal_template"
run_install update
grep -qx '/\* user-owned \*/' "$portal_template"

run_install update --enable all
grep -qx -- '--user daemon-reload' "$test_dir/systemctl.log"
grep -qx -- '--user enable --now caelestia-extras-watch.service' "$test_dir/systemctl.log"
grep -Fqx "ExecStart=\"$test_dir/bin/caelestia-extras\" --config \"$config_file\" watch" "$test_dir/config/caelestia-extras/managed/systemd/caelestia-extras-watch.service"
test -L "$test_dir/config/environment.d/10-caelestia-qt.conf"
test ! -e "$test_dir/config/systemd/user/caelestia-extras-gtk.path"
test ! -e "$test_dir/config/systemd/user/caelestia-extras-qt.path"

export CAELESTIA_EXTRAS_SCHEME_FILE="$test_dir/custom/scheme.json"
export CAELESTIA_EXTRAS_THEME_DIR="$test_dir/custom/theme"
export CAELESTIA_EXTRAS_PORTAL_THEME_NAME="Manual-Portal"
run_install update
grep -Fqx "ExecStart=\"$test_dir/bin/caelestia-extras\" --config \"$config_file\" watch" "$test_dir/config/caelestia-extras/managed/systemd/caelestia-extras-watch.service"
grep -Fqx "Environment=GTK_THEME=Manual-Portal" "$test_dir/config/caelestia-extras/managed/systemd/xdg-desktop-portal-gtk.service.d/10-caelestia-theme.conf"
grep -Fqx "Environment=GTK_THEME=Manual-Portal" "$test_dir/config/caelestia-extras/managed/systemd/xdg-desktop-portal-gnome.service.d/10-caelestia-theme.conf"
test -L "$test_dir/data/themes/Manual-Portal/gtk-3.0/base-dark.css"
